...
describe('dc.sunburstChart', () => {
    ...
    describe('generation', () => {
        ...
        it('slice path fill should be set correctly', () => {
            expect(d3.select(chart.selectAll('g.pie-slice path').nodes()[0]).attr('fill')).toEqual(
                '#3182bd'
            );
            expect(d3.select(chart.selectAll('g.pie-slice path').nodes()[1]).attr('fill')).toEqual(
                '#6baed6'
            );
            expect(d3.select(chart.selectAll('g.pie-slice path').nodes()[2]).attr('fill')).toEqual(
                '#9ecae1'
            );
            expect(d3.select(chart.selectAll('g.pie-slice path').nodes()[3]).attr('fill')).toEqual(
                '#c6dbef'
            );
        });
        ...
        describe('slice selection', () => {
            ...
            it('be able to set selected slice', () => {
                expect(chart.filter(['US', 'East', 'Ontario']).filter()).toEqual([
                    'US',
                    'East',
                    'Ontario',
                ]);
            ...
            it('should highlight selected slices', () => {
                ...
                chart.selectAll('g.pie-slice-level-3').each(function (d) {
                    if (
                        d.path.toString() === ['CA', 'East', 'Ontario'].toString() ||
    ...
    describe('sunburst.relativeRingSizes', () => {
        ...
        function buildSunburstChartNCompleteRings(N, id) {
            ...
            const _valueDimension = data.dimension(d => {
                ...
                const key = Array.prototype.concat.apply(
                    ten.slice(0, N % 10),
                    new Array(Math.floor(N / 10)).fill(ten)
                );
        ...
        describe('sunburst.defaultRingSizes: shrinking', () => {
            ...
            it('rings should get narrower, farther away from the center', () => {
                ...
                expect(getRingThicknessRounded(chart, 1)).toBeGreaterThan(
                    getRingThicknessRounded(chart, 2)
                );
            });
        });
        ...
        describe('sunburst.relativeRingSizes: equal distribution', () => {
            let chart;
            beforeEach(() => {
                chart = buildSunburstChart3CompleteRings(
                    'sunburst_relativeRingSizes_equal_distribution'
                );
            ...
            it('rings should be equally wide', () => {
                ...
                expect(getRingThicknessRounded(chart, 1)).toEqual(
                    getRingThicknessRounded(chart, 2)
                );
            });
        });
        ...
        function testEqualRings(N) {
            describe(`sunburst.relativeRingSizes: equal distribution - no rounding errors with ${N} rings`, () => {
                let chart;
                beforeEach(() => {
                    chart = buildSunburstChartNCompleteRings(
                        N,
                        'sunburst_relativeRingSizes_equal_distribution_10rings'
                    );
                    chart.ringSizes(chart.equalRingSizes());
                });
        ...
        describe('sunburst.relativeRingSizes: specific percentages', () => {
            ...
            beforeEach(() => {
                chart = buildSunburstChart3CompleteRings(
                    'sunburst_relativeRingSizes_specific_percentages'
                );
            ...
            it('2nd ring should be half as wide as the 3rd ', () => {
                expect(getRingThicknessRounded(chart, 1)).toBeGreaterThan(0);
                expect(2 * getRingThicknessRounded(chart, 1)).toEqual(
                    getRingThicknessRounded(chart, 2)
                );
            });
        });
        ...
        describe('sunburst.relativeRingSizes: invalid arguments', () => {
            ...
            beforeEach(() => {
                chart = buildSunburstChart3CompleteRings(
                    'sunburst_relativeRingSizes_invalid_arguments'
                );
            });
            ...
            it('invalid arguments cause dc.errors.BadArgumentException, default function does not', () => {
                ...
                expect(() => {
                    chart.render();
                }).toThrowError(dc.BadArgumentException);

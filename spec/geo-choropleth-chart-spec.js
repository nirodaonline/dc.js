...
describe('dc.geoChoropleth', () => {
    ...
    function buildChartWithCustomProjection(id) {
        ...
        chart
            ...
            .projection(d3.geoMercator().scale(26778).translate([8227, 3207]))
            .width(990)
            .height(600)
            .colors([
                '#ccc',
    ...
    describe('creation', () => {
        ...
        it('correct css class should be set [Alaska]', () => {
            expect(chart.selectAll('g.layer0 g.state').nodes()[1].getAttribute('class')).toEqual(
                'state alaska'
            );
        });
        it('correct title should be set [Alaska]', () => {
            expect(chart.selectAll('g.layer0 g.state title').nodes()[1].textContent).toEqual(
                'Alaska : 0'
            );
        });
        it('correct color filling should be set [Alaska]', () => {
            expect(
                chart.selectAll('g.layer0 g.state path').nodes()[1].getAttribute('fill')
            ).toMatch(/#ccc/i);
        });
        it('correct state boundary should be rendered [Alaska]', () => {
            expect(
                chart.selectAll('g.layer0 g.state path').nodes()[1].getAttribute('d').length
            ).not.toEqual(0);
        });
        it('correct css class should be set [California]', () => {
            expect(chart.selectAll('g.layer0 g.state').nodes()[4].getAttribute('class')).toEqual(
                'state california'
            );
        });
        it('correct css class should be set [District of Columbia]', () => {
            expect(chart.selectAll('g.layer0 g.state').nodes()[8].getAttribute('class')).toEqual(
                'state district_of_columbia'
            );
        });
        it('correct title should be set [California]', () => {
            expect(chart.selectAll('g.layer0 g.state title').nodes()[4].textContent).toEqual(
                'California : 154'
            );
        });
        it('correct color should be set [California]', () => {
            expect(
                chart.selectAll('g.layer0 g.state path').nodes()[4].getAttribute('fill')
            ).toMatch(/#0089ff/i);
        });
        it('correct state boundary should be rendered [California]', () => {
            expect(
                chart.selectAll('g.layer0 g.state path').nodes()[4].getAttribute('d').length
            ).not.toEqual(0);
        });
        it('correct css class should be set [Colorado]', () => {
            expect(chart.selectAll('g.layer0 g.state').nodes()[5].getAttribute('class')).toEqual(
                'state colorado'
            );
        });
        it('correct title should be set [Colorado]', () => {
            expect(chart.selectAll('g.layer0 g.state title').nodes()[5].textContent).toEqual(
                'Colorado : 22'
            );
        });
        it('correct color should be set [Colorado]', () => {
            expect(
                chart.selectAll('g.layer0 g.state path').nodes()[5].getAttribute('fill')
            ).toMatch(/#e2f2ff/i);
        });
        it('correct state boundary should be rendered [Colorado]', () => {
            expect(
                chart.selectAll('g.layer0 g.state path').nodes()[5].getAttribute('d').length
            ).not.toEqual(0);
        });
        ...
        it('correct css class should be set [county]', () => {
            expect(chart.selectAll('g.layer1 g.county').nodes()[1].getAttribute('class')).toEqual(
                'county'
            );
        });
        ...
        it('correct color filling should be set [county]', () => {
            expect(
                chart.selectAll('g.layer1 g.county path').nodes()[1].getAttribute('fill')
            ).toEqual('white');
        });
        it('correct state boundary should be rendered [county]', () => {
            expect(
                chart.selectAll('g.layer1 g.county path').nodes()[1].getAttribute('d').length
            ).not.toEqual(0);
        });
    });
    ...
    describe('filter and highlight', () => {
        ...
        it('sets deselected classes for some states', () => {
            expect(chart.selectAll('g.layer0 g.state').nodes()[0].getAttribute('class')).toEqual(
                'state alabama deselected'
            );
            expect(chart.selectAll('g.layer0 g.state').nodes()[1].getAttribute('class')).toEqual(
                'state alaska deselected'
            );
        });
        it('sets selected classes for selected states', () => {
            expect(chart.selectAll('g.layer0 g.state').nodes()[4].getAttribute('class')).toEqual(
                'state california selected'
            );
            expect(chart.selectAll('g.layer0 g.state').nodes()[5].getAttribute('class')).toEqual(
                'state colorado selected'
            );
        });
    });
    ...
    describe('respond to external filter', () => {
        ...
        it('gets right colors when not filtered', () => {
            expect(chart.selectAll('g.layer0 g.state.california path').attr('fill')).toMatch(
                /#0089ff/i
            );
            expect(chart.selectAll('g.layer0 g.state.colorado path').attr('fill')).toMatch(
                /#e2f2ff/i
            );
            expect(chart.selectAll('g.layer0 g.state.delaware path').attr('fill')).toMatch(
                /#c4e4ff/i
            );
            expect(chart.selectAll('g.layer0 g.state.mississippi path').attr('fill')).toMatch(
                /#81c5ff/i
            );
            expect(chart.selectAll('g.layer0 g.state.oklahoma path').attr('fill')).toMatch(
                /#9ed2ff/i
            );
            expect(chart.selectAll('g.layer0 g.state.maryland path').attr('fill')).toMatch(/#ccc/i);
            expect(chart.selectAll('g.layer0 g.state.washington path').attr('fill')).toMatch(
                /#ccc/i
            );
        });
        it('has right titles when not filtered', () => {
            expect(chart.selectAll('g.layer0 g.state.california title').text()).toEqual(
                'California : 154'
            );
            expect(chart.selectAll('g.layer0 g.state.colorado title').text()).toEqual(
                'Colorado : 22'
            );
            expect(chart.selectAll('g.layer0 g.state.delaware title').text()).toEqual(
                'Delaware : 33'
            );
            expect(chart.selectAll('g.layer0 g.state.mississippi title').text()).toEqual(
                'Mississippi : 77'
            );
            expect(chart.selectAll('g.layer0 g.state.oklahoma title').text()).toEqual(
                'Oklahoma : 55'
            );
            expect(chart.selectAll('g.layer0 g.state.maryland title').text()).toEqual(
                'Maryland : 0'
            );
            expect(chart.selectAll('g.layer0 g.state.washington title').text()).toEqual(
                'Washington : 0'
            );
        });
    ...
    describe('replace and remove layer', () => {
        ...
        it('geo json layer with the same name should be replaced', () => {
            expect(chart.geoJsons().filter(e => e.name === 'state')[0].data).toBe(
                geoJson3.features
            );
        });
